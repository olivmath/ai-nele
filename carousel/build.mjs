...
const dataDir = join(root, 'carousel', 'data')
const outDir = join(root, 'carousel', 'dist')
...
await build({entryPoints:[join(root,'carousel','src','main.jsx')],outfile:join(outDir,'app.js'),bundle:true,format:'iife',platform:'browser',jsx:'automatic',minify:true})
for (const [key,[slug,name,report]] of Object.entries(manifest)) {
  const dataPath=join(dataDir,`${slug}.json`); let carousel
  try { carousel=JSON.parse(await readFile(dataPath,'utf8')) } catch { const legacy=await readFile(join(root,`carousel-hack-${key}.html`),'utf8'); carousel={slug,name,report,slides:extractSlides(legacy)}; await writeFile(dataPath,JSON.stringify(carousel,null,2)) }
  const safe=JSON.stringify(carousel).replace(/</g,'\\u003c')
  await writeFile(join(root,`carousel-hack-${key}.html`),`<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>${name} — Security Incident</title><link rel="stylesheet" href="carousel/dist/app.css"></head><body><div id="root"></div><script>window.CAROUSEL_DATA=${safe}</script><script src="carousel/dist/app.js"></script></body></html>`)
