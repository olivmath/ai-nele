...
function Trail({ index }) {
  return (
    <aside className="incident-trail" aria-hidden="true">
      <span className="trail-marker" />
      <span className="trail-count">{String(index + 1).padStart(2, '0')}</span>
    </aside>
  )
}
...
function Slide({ html, index }) {
  return (
    <article className={`slide report-slide slide-${index + 1}`} aria-label={`Slide ${index + 1}`}>
      <Trail index={index} />
      <Card className="slide-content" dangerouslySetInnerHTML={{ __html: html }} />
    </article>
  )
}

export function CarouselViewer({ carousel }) {
  ...
  return (
    <main className="carousel" aria-label={`${carousel.name} incident report`}>
      <div className="report-ribbon">
        <Badge>SECURITY INCIDENT</Badge>
      ...
      <nav className="nav" aria-label="Carousel navigation">
        <Button aria-label="Previous slide" disabled={active === 0} onClick={() => goTo(active - 1)}>
          ←
        </Button>
        <div className="dots">
        ...
        <Button aria-label="Next slide" disabled={active === total - 1} onClick={() => goTo(active + 1)}>
          →
        </Button>
      </nav>
    </main>
